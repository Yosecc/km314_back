...
class FormControlMonitor extends Page
{
    ...
    public function adminApproveAction(): Action
    {
        return Action::make('adminApprove')->label('Aprobar')->icon('heroicon-o-check')->color('success')->size('sm')
            ...
            ->requiresConfirmation()->action(function (FormControl $record): void {
                ...
                $record->aprobar(); $this->notifyOwner($record,'Formulario aprobado','Las personas configuradas ya pueden acceder según los horarios establecidos.');
                unset($this->monitorData); Notification::make()->title('Formulario aprobado')->success()->send();
            });
    }
    ...
    public function rejectAction(): Action
    {
        return Action::make('reject')->label('Rechazar')->icon('heroicon-o-x-circle')->color('danger')->size('sm')
            ...
            ->requiresConfirmation()->action(function (FormControl $record): void {
                ...
                $record->rechazar(); $this->notifyOwner($record,'Formulario rechazado','Administración rechazó el formulario #'.$record->id.'.');
                unset($this->monitorData); Notification::make()->title('Formulario rechazado')->success()->send();
            });
    }
    ...
    {
        $forms = $this->baseQuery()->with(['owner','peoples','dateRanges'])->latest()->limit(400)->get()->map(fn (FormControl $form) => $this->mapForm($form));
        ...
        $visible = $forms->when($this->status === 'active',fn (Collection $rows) => $rows->whereIn('computed_status',['OwnerPending','Pending','Authorized','Vencido']))
            ->when($this->status === 'attention',fn (Collection $rows) => $rows->whereIn('computed_status',['Denied','Vencido','Expirado']))
        ...
        return ['forms'=>$visible,'stats'=>[
            ...
            'attention'=>$forms->whereIn('computed_status',['Denied','Vencido','Expirado'])->count(),
    ...
    {
        $status = $form->statusComputed(); $range = $form->dateRanges->first();
        $startDate=$range?->start_date_range ?: $form->start_date_range; $startTime=$range?->start_time_range ?: $form->start_time_range;
        $endDate=$range?->end_date_range ?: $form->end_date_range; $endTime=$range?->end_time_range ?: $form->end_time_range;
    ...
    private function findVisibleForm(int $id): FormControl { return $this->baseQuery()->with(['owner','peoples','dateRanges'])->findOrFail($id); }
    private function findVisibleFormIncludingDeleted(int $id): FormControl { return $this->baseQuery()->withTrashed()->with(['owner','peoples','dateRanges'])->findOrFail($id); }
    private function isAdmin(): bool { return auth()->user()->hasAnyRole(['super_admin','admin','Administrador']); }
    private function notifyOwner(FormControl $record,string $title,string $body): void
    {
        if ($record->owner?->user) Notification::make()->title($title)->body($body)->actions([NotificationAction::make('ver')->label('Ver')->url(FormControlResource::getUrl('view',['record'=>$record]))])->sendToDatabase($record->owner->user);
    }
}
